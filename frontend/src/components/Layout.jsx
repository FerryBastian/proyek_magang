...
export default function Layout({ children }) {
  ...
  const adminMenus = [
    { label: "Dashboard", tab: "dashboard", icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><rect x="3" y="3" width="7" height="7" rx="1" stroke="currentColor" strokeWidth="2"/><rect x="14" y="3" width="7" height="7" rx="1" stroke="currentColor" strokeWidth="2"/><rect x="3" y="14" width="7" height="7" rx="1" stroke="currentColor" strokeWidth="2"/><rect x="14" y="14" width="7" height="7" rx="1" stroke="currentColor" strokeWidth="2"/></svg> },
    { label: "Pengajuan", tab: "submissions", icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg> },
    { label: "Workshop", tab: "workshops", icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z" stroke="currentColor" strokeWidth="2"/><polyline points="9 22 9 12 15 12 15 22" stroke="currentColor" strokeWidth="2"/></svg> },
    { label: "Divisi", tab: "divisions", icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2" stroke="currentColor" strokeWidth="2"/><circle cx="9" cy="7" r="4" stroke="currentColor" strokeWidth="2"/><path d="M23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75" stroke="currentColor" strokeWidth="2"/></svg> },
    { label: "Users", tab: "users", icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="8" r="4" stroke="currentColor" strokeWidth="2"/><path d="M4 20c0-4 3.6-7 8-7s8 3 8 7" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg> },
  ];
  ...
  const globalStyles = `
    ...
    .dtech-nav a{color:#fff!important}
    .dtech-nav a:hover{background:rgba(255,255,255,0.15)!important}
    .mobile-menu-item:hover{background:rgba(255,255,255,0.1)!important}
    .avatar-btn{transition:all 0.2s ease;outline:none}
    .avatar-btn:hover{transform:scale(1.04);box-shadow:0 0 0 3px rgba(0,180,216,0.4)!important}
    .profile-dropdown{animation:dropdownIn 0.22s cubic-bezier(0.34,1.56,0.64,1) forwards}
    .dd-item{transition:background 0.15s;cursor:pointer;border:none;background:transparent;width:100%;text-align:left;font-family:inherit;display:flex;align-items:center;gap:10px}
    .dd-item:hover{background:rgba(0,150,199,0.07)!important}
    ...
    <div ref={dropdownRef} style={{ position: "relative" }}>
      <button
        ...
        style={{
          ...
          background: profileDropdown ? "rgba(255,255,255,0.25)" : "rgba(255,255,255,0.15)",
          border: "1.5px solid rgba(255,255,255,0.3)",
          cursor: "pointer",
        }}
      >
        <div style={{ width: 34, height: 34, borderRadius: "50%", overflow: "hidden", flexShrink: 0, background: "linear-gradient(135deg,#00B4D8,#0077A8)", display: "flex", alignItems: "center", justifyContent: "center", border: "2px solid rgba(255,255,255,0.5)" }}>
          {user.avatar
            ? <img src={user.avatar} alt={user.name} referrerPolicy="no-referrer" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
            : <span style={{ color: "#fff", fontSize: 14, fontWeight: 700 }}>{user.name?.charAt(0).toUpperCase() || "U"}</span>
          }
        </div>
        <div style={{ textAlign: "left" }}>
          <div style={{ color: "#fff", fontSize: 13, fontWeight: 600, lineHeight: 1.2 }}>{user.name?.split(" ")[0] || user.email?.split("@")[0]}</div>
          <div style={{ color: "rgba(255,255,255,0.55)", fontSize: 10, textTransform: "capitalize" }}>{user.role}</div>
        </div>
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" style={{ color: "rgba(255,255,255,0.65)", transition: "transform 0.2s", transform: profileDropdown ? "rotate(180deg)" : "rotate(0deg)", flexShrink: 0 }}>
          <path d="M6 9l6 6 6-6" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
        </svg>
      </button>

      {profileDropdown && (
        <div className="profile-dropdown" style={{ position: "absolute", top: "calc(100% + 10px)", right: 0, width: 224, background: "#fff", borderRadius: 16, boxShadow: "0 20px 54px rgba(0,40,80,0.18),0 0 0 1px rgba(0,150,199,0.1)", overflow: "hidden", zIndex: 500 }}>
          {/* Header strip */}
          <div style={{ padding: "14px 16px 12px", background: "linear-gradient(135deg,#002d45,#0077A8)", display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 40, height: 40, borderRadius: "50%", overflow: "hidden", background: "rgba(255,255,255,0.2)", border: "1.5px solid rgba(255,255,255,0.4)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              {user.avatar
                ? <img src={user.avatar} alt="" referrerPolicy="no-referrer" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                : <span style={{ color: "#fff", fontSize: 16, fontWeight: 700 }}>{user.name?.charAt(0).toUpperCase()}</span>
              }
            </div>
          ...
          <div style={{ padding: "6px" }}>
            <button className="dd-item" onClick={() => { setProfileDropdown(false); setShowProfileModal(true); }} style={{ padding: "10px 12px", borderRadius: 10 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 11, color: "#9CA3AF", marginTop: 1 }}>No. telp, workshop, divisi</div>
              </div>
            </button>
            <div style={{ height: 1, background: "#f0f7fb", margin: "4px 6px" }} />
            <button className="dd-item dd-item-danger" onClick={() => { setProfileDropdown(false); setShowLogoutModal(true); }} style={{ padding: "10px 12px", borderRadius: 10 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 11, color: "#9CA3AF", marginTop: 1 }}>Keluar dari akun</div>
              </div>
            </button>
          </div>
        </div>
  ...
  const LogoutModal = () => showLogoutModal ? (
    <div onClick={() => setShowLogoutModal(false)} style={{ position: "fixed", inset: 0, zIndex: 1050, background: "rgba(15,10,40,0.45)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24, animation: "backdropIn 0.2s ease forwards" }}>
      <div onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: 20, padding: "36px 28px", width: "100%", maxWidth: 400, textAlign: "center", boxShadow: "0 24px 64px rgba(0,119,168,0.2)", border: "1px solid #cce6f0", animation: "modalIn 0.25s ease forwards" }}>
  ...
  if (user?.role === "admin") {
    const SidebarContent = ({ onClose }) => (
      <div style={{ display: "flex", flexDirection: "column", height: "100%", background: "linear-gradient(180deg,#001a2e 0%,#003f5c 50%,#0077A8 100%)" }}>
        <div style={{ padding: "20px", borderBottom: "1px solid rgba(255,255,255,0.1)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          {onClose && <button onClick={onClose} style={{ background: "rgba(255,255,255,0.1)", border: "none", borderRadius: 8, width: 30, height: 30, color: "#fff", cursor: "pointer", fontSize: 18, display: "flex", alignItems: "center", justifyContent: "center" }}>×</button>}
        </div>
        <div style={{ padding: "14px 20px", borderBottom: "1px solid rgba(255,255,255,0.1)", display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 38, height: 38, borderRadius: "50%", background: "rgba(255,255,255,0.2)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 15, fontWeight: 700, flexShrink: 0, border: "1.5px solid rgba(255,255,255,0.3)" }}>{user.name?.charAt(0).toUpperCase() || "A"}</div>
          <div style={{ minWidth: 0 }}>
        ...
        <div style={{ flex: 1, padding: "12px", display: "flex", flexDirection: "column", gap: 2, overflowY: "auto" }}>
          {adminMenus.map(menu => {
            const isActive = menu.label === "Dashboard" ? location.pathname === "/admin" && !location.search : currentTab === menu.tab;
            return (
    ...
    return (
      <div style={{ display: "flex", minHeight: "100vh", fontFamily: "'Sora','Barlow',sans-serif", background: "#EBF6FA" }}>
        <style>{globalStyles}{`.admin-sidebar{display:flex;width:230px;flex-shrink:0;position:sticky;top:0;height:100vh;box-shadow:4px 0 20px rgba(0,0,0,0.15)}.admin-mobile-header{display:none}@media(max-width:768px){.admin-sidebar{display:none!important}.admin-mobile-header{display:flex!important}.admin-topbar{padding:0 16px!important}.admin-content{padding:16px!important}}`}</style>
        <div className="admin-sidebar" style={{ flexDirection: "column" }}><SidebarContent /></div>
        <div className="admin-mobile-header" style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, background: "linear-gradient(90deg,#001a2e,#0077A8)", height: 56, alignItems: "center", padding: "0 16px", gap: 12, boxShadow: "0 2px 12px rgba(0,0,0,0.2)" }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
          <div className="admin-topbar" style={{ background: "#fff", padding: "0 24px", height: 60, borderBottom: "1px solid #d4eef8", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 40, boxShadow: "0 2px 8px rgba(0,150,199,0.06)" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "5px 12px", borderRadius: 20, background: "#EBF6FA", border: "1px solid #d4eef8" }}>
              <div style={{ width: 26, height: 26, borderRadius: "50%", background: "linear-gradient(135deg,#0096C7,#00B4D8)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 11, fontWeight: 700 }}>{user.name?.charAt(0).toUpperCase() || "A"}</div>
              <span style={{ fontSize: 13, fontWeight: 600, color: "#0D3040" }}>{user.name || "Admin"}</span>
              <span style={{ fontSize: 10, padding: "2px 7px", borderRadius: 10, background: "#0096C7", color: "#fff", fontWeight: 600 }}>Admin</span>
            </div>
          </div>
          <main className="admin-content" style={{ flex: 1, padding: "24px", overflowY: "auto" }}>{children}</main>
        </div>
        <LogoutModal />
      </div>
    );
  ...
  return (
    <div className="min-h-screen" style={{ background: "#EBF6FA" }}>
      <style>{globalStyles}{`@media(max-width:640px){.user-nav-desktop{display:none!important}.user-nav-mobile-btn{display:flex!important}}.user-nav-mobile-btn{display:none}@keyframes slideDown{from{opacity:0;transform:translateY(-8px)}to{opacity:1;transform:translateY(0)}}.mobile-menu-dropdown{animation:slideDown 0.2s ease forwards}`}</style>
      ...
      <nav className="dtech-nav border-b shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16">
            ...
            <div className="user-nav-desktop flex items-center gap-3">
              {user?.role === "user" && (
              ...
              {!user && (
                <Link to="/login" className="ml-2 px-4 py-2 rounded-lg font-medium" style={{ background: "#00B4D8", color: "#000" }}>Login</Link>
              )}
              {user && <AvatarDropdown />}
            </div>
          ...
          <div className="mobile-menu-dropdown" style={{ background: "rgba(0,26,46,0.97)", borderTop: "1px solid rgba(255,255,255,0.1)", padding: "12px 16px", display: "flex", flexDirection: "column", gap: 4 }}>
            {user && (
              <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 12px", background: "rgba(255,255,255,0.08)", borderRadius: 10, marginBottom: 6 }}>
                <div style={{ width: 36, height: 36, borderRadius: "50%", overflow: "hidden", background: "#00B4D8", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, fontWeight: 700, color: "#fff", flexShrink: 0 }}>
                  {user.avatar ? <img src={user.avatar} alt="" referrerPolicy="no-referrer" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : user.name?.charAt(0).toUpperCase()}
                </div>
                <div>
            ...
            {user?.role === "user" && (
              <Link to="/user" onClick={() => setMobileMenuOpen(false)} className="mobile-menu-item"
                style={{ display: "flex", alignItems: "center", gap: 10, padding: "11px 12px", borderRadius: 10, color: "#fff", textDecoration: "none", fontSize: 14, fontWeight: 500 }}>
            ...
            {!user && (
              <Link to="/login" onClick={() => setMobileMenuOpen(false)}
                style={{ display: "flex", alignItems: "center", gap: 10, padding: "11px 12px", borderRadius: 10, background: "#00B4D8", color: "#000", textDecoration: "none", fontSize: 14, fontWeight: 600 }}>
                Login
              </Link>
            )}
            {user && (
              <>
                <button onClick={() => { setMobileMenuOpen(false); setShowProfileModal(true); }} className="mobile-menu-item"
                  style={{ display: "flex", alignItems: "center", gap: 10, padding: "11px 12px", borderRadius: 10, background: "transparent", border: "none", color: "#fff", cursor: "pointer", fontSize: 14, fontWeight: 500, width: "100%", textAlign: "left", fontFamily: "inherit" }}>
                  <span style={{ fontSize: 17 }}>👤</span> Profil Saya
                </button>
                <button onClick={() => { setShowLogoutModal(true); setMobileMenuOpen(false); }}
                  style={{ display: "flex", alignItems: "center", gap: 10, padding: "11px 12px", borderRadius: 10, background: "rgba(239,68,68,0.15)", border: "none", color: "#FCA5A5", cursor: "pointer", fontSize: 14, fontWeight: 500, width: "100%", textAlign: "left", fontFamily: "inherit" }}>
